...
// 			$model->imageFiles = UploadedFile::getInstances ( $model, "imageFiles" );
	...
	protected function renderUpdate($model) {
		...
		if ($model->cover_thumb_url) {
			$initialPreviewCover [] = UrlUtils::formatUrl ( $model->cover_thumb_url );
		}
	...
	public function actionDelete($id) {
		...
		try {
			// 删除图片
			$vipCasePhotos = VipCasePhoto::find ()->where ( [ 
					'case_id' => $id 
			] )->all ();
			foreach ( $vipCasePhotos as $vipCasePhoto ) {
	...
	protected function findModel($id) {
		$model = VipCase::find ()->alias ( 'case' )->joinWith ( 'auditUser auditUser' )->joinWith ( 'auditStatus auditStatus' )->joinWith ( 'type type' )->joinWith ( 'caseFlag caseFlag' )->joinWith ( 'status0 stat' )->joinWith ( 'status0 isHot' )->joinWith ( 'vip vip' )->where ( [ 
				'case.id' => $id 
		] )->one ();
		if ($model !== null) {
	...
	protected function findVipCaseTypeByVipTypeId($vip_type_id) {
		$model = VipCaseType::find ()->where ( [ 
				'vip_type_id' => $vip_type_id 
		] )->one ();
		return $model;
	}
